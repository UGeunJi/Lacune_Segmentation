...
class LPNetGA(nn.Module):
	def __init__(self, im_size:tuple, ksize:tuple=(32,32), stride:tuple=(16,16), fuse_coe=1.0):
	...
	def forward(self, x, mode=None):
		...
		padding_fm = torch.zeros((b,c,ph,pw)).to(x.device)
		...
		fused_fm = torch.zeros((b,1,ph,pw)).to(x.device)
		...
		def subFuse(addIm, backIm, fuse_state):
			...
			backIm = torch.tensor(backIm).to(x.device)
		...
		for i in range(0,h+self.ksize[0]+1,self.stride[0]):
			for j in range(0,w+self.ksize[1]+1,self.stride[1]):
				...
				sub = self.lfe(sub) # patch-likelihood map
