...
class RLKunet(nn.Module):
    ...
    def forward(self, x):                       # batch, 1, H, W
        ...
        up3         = self.up3(enc4_4)                  # batch, 128, H/4, W/4
        concat3     = torch.cat((enc3_2, up3), dim=1)   # batch, 128+64, H/4, W/4
        dec3_1      = self.decoder3_1(concat3)          # batch, 64, H/4, W/4
        dec3_2      = self.decoder3_2(dec3_1)

        up2         = self.up2(dec3_2)                  # batch, 64, H/2, W/2
        concat2     = torch.cat((enc2_2, up2), dim=1)   # batch, 64+32, H/2, W/2
        dec2_1      = self.decoder2_1(concat2)          # batch, 32, H/2, W/2
        dec2_2      = self.decoder2_2(dec2_1)

        up1         = self.up1(dec2_2)                  # batch, 32, H, W
        concat1     = torch.cat((enc1_2, up1), dim=1)   # batch, 32+16, H, W
        dec1_1      = self.decoder1_1(concat1)          # batch, 16, H, W
