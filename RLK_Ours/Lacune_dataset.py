...
class Lacune_fold_raw_25D_dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        flair_img_up, flair_img, flair_img_down, label_img = self.slices_list[index]
        
        flair_img = normalize(flair_img)
        flair_img_up = normalize(flair_img_up)
        flair_img_down = normalize(flair_img_down)
        
        flair_img = np.nan_to_num(flair_img)
        flair_img_up = np.nan_to_num(flair_img_up)
        flair_img_down = np.nan_to_num(flair_img_down)
        
        x_img = np.stack([flair_img_up, flair_img, flair_img_down], axis=0) # (3, 224, 224)
        x_img = (x_img * 255).astype(np.uint8)
        x_img = x_img / 255
