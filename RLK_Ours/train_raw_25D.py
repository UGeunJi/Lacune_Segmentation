...
import torch
from Lacune_dataset import Lacune_fold_raw_25D_dataset
...
def extract_lacune_slices(train_flair_files, train_label_files):
    ...
    for i in range(len(train_flair_files)):
        ...
        for j in range(label.shape[2]):
            if np.max(label[:, :, j]) > 0.0 and j > 0:
                slices_with_lacune.append((flair[:, :, j-1], flair[:, :, j], flair[:, :, j+1], label[:, :, j]))
...
weights_path    = "/nasdata4/mjh/Diffusion/2_Segmentation/1_RLK_final/4_RLK_Ours/weights_raw/weight_density_multi_lambda_5fold_preset_raw_25D_lrschdule"
...
for fold in range(folds_num):
    ...
    train_lacune_slices = extract_lacune_slices(train_fold_ith_flair_list, train_fold_ith_label_list)
    val_lacune_slices = extract_lacune_slices(val_fold_ith_flair_list, val_fold_ith_label_list)
    
    train_dataset = Lacune_fold_raw_25D_dataset(train_lacune_slices, transform=transform)
    val_dataset = Lacune_fold_raw_25D_dataset(val_lacune_slices, transform=transform_val)
    ...
    scheduler = ReduceLROnPlateau(
        optimizer, mode='max', factor=0.5, patience=10, threshold=0.01, min_lr=1e-6
    )
    ...
    for epoch in range(epochs):
        ...
        mean_f1_score = (tarf1 + pxlf1 + avg_f1) / 3
        scheduler.step(mean_f1_score)
